...
export default function Overview({ camps }) {
  return (
    <>
          ...
          return (
            <div key={i} style={{ ...cardStyle, padding: "20px 22px", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 20, background: "rgba(52,168,83,0.08)", color: C.ok, fontSize: 12, fontWeight: 500 }}>
                  <ArrowUpRight size={13} />{m.ch}
                </div>
              </div>
              <div style={{ fontSize: 28, fontWeight: 500, fontVariantNumeric: "tabular-nums" }}>{m.vl}</div>
              <div style={{ fontSize: 13, color: C.tt, marginTop: 4 }}>{m.lb}</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16, marginBottom: 24 }}>
        ...
        <div style={{ ...cardStyle, padding: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
            <div style={{ fontSize: 15, fontWeight: 600 }}>Динамика эффективности</div>
            <div style={{ display: "flex", gap: 16 }}>
              {[{ l: "Клики", c: G.b }, { l: "Конверсии", c: G.g }].map(x => (
                <div key={x.l} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: C.tt }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={chartData}>
              ...
              <XAxis dataKey="d" tick={{ fill: C.tt, fontSize: 12 }} axisLine={false} tickLine={false} />
              <YAxis yAxisId="l" tick={{ fill: C.tt, fontSize: 12 }} axisLine={false} tickLine={false} />
              <YAxis yAxisId="r" orientation="right" tick={{ fill: C.tt, fontSize: 12 }} axisLine={false} tickLine={false} />
        ...
        {/* Pie chart */}
        <div style={{ ...cardStyle, padding: 24 }}>
          <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 16 }}>Каналы трафика</div>
            ...
            {channels.map((ch, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 13, color: C.ts }}>{ch.n}</span>
                </div>
                <span style={{ fontSize: 13, fontWeight: 500, fontVariantNumeric: "tabular-nums" }}>{ch.v}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ ...cardStyle, padding: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
          <div style={{ fontSize: 15, fontWeight: 600 }}>Кампании</div>
          <div style={{ fontSize: 12, color: C.tt }}>Обновлено 5 мин назад</div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 0.8fr 1.2fr 0.8fr 0.8fr 0.5fr", padding: "0 0 14px", borderBottom: "1px solid " + C.bd }}>
          {["Название", "Статус", "Бюджет / Расход", "CPA", "ROAS", ""].map(h => (
            <div key={h} style={{ fontSize: 11, fontWeight: 500, color: C.tt, textTransform: "uppercase", letterSpacing: "0.06em" }}>{h}</div>
          ))}
        </div>
        {camps.filter(c => c.s !== "draft").map((c, i) => (
          <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 0.8fr 1.2fr 0.8fr 0.8fr 0.5fr", padding: "15px 0", borderBottom: "1px solid " + C.bd, alignItems: "center" }}>
            ...
            <div>
              <span style={{ fontSize: 11, fontWeight: 500, padding: "4px 11px", borderRadius: 20, background: c.s === "active" ? "rgba(52,168,83,0.08)" : C.sa, color: c.s === "active" ? C.ok : C.tt }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 500, fontVariantNumeric: "tabular-nums", color: cpaColor(c.cpa) }}>{c.cpa}₽</div>
            <div style={{ fontSize: 14, fontWeight: 500, fontVariantNumeric: "tabular-nums", color: roasColor(c.roas) }}>{c.roas}x</div>
            <div>
              {c.t === "up"
