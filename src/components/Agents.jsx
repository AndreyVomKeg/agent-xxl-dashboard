  ...
  return (
    <>
        ...
        {agents.map((a, i) => {
          ...
          return (
            <div key={i} onClick={() => setExpanded(expanded === a.nm ? null : a.nm)}
              ...
              <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 2 }}>{a.nm}</div>
              <div style={{ fontSize: 12, color: C.tt }}>{a.imp} · {(skills[a.nm] || []).length} скиллов</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ ...cardStyle, padding: 24, marginBottom: 24 }}>
        <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 20 }}>Эффективность агентов</div>
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={agentPerformance} barGap={4}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(60,40,20,0.05)" />
            <XAxis dataKey="name" tick={{ fill: C.tt, fontSize: 12 }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: C.tt, fontSize: 12 }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={{ background: "#FFF", border: "1px solid " + C.bd, borderRadius: 10, fontSize: 13 }} />
        ...
        return (
          <div key={i} style={{ ...cardStyle, marginBottom: 16, overflow: "hidden" }}>
            ...
            <div onClick={() => setExpanded(isE ? null : a.nm)}
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 500 }}>{a.nm}</div>
                <div style={{ fontSize: 12, color: C.tt }}>{a.ds}</div>
              </div>
              <StatusDot status={a.st} />
                ...
                <div style={{ padding: 20 }}>
                  ...
                  {subTab === "skills" && (
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                      <div>
                        <div style={{ fontSize: 12, fontWeight: 500, color: C.tt, marginBottom: 8 }}>ТЕКУЩАЯ ЗАДАЧА</div>
                        <div style={{ padding: "10px 14px", borderRadius: 10, background: a.co + "08", border: "1px solid " + a.co + "20", fontSize: 13, marginBottom: 14 }}>{a.tk}</div>
                        <div style={{ fontSize: 12, fontWeight: 500, color: C.tt, marginBottom: 8 }}>ПОСЛЕДНИЕ ДЕЙСТВИЯ</div>
                        {a.acts.map((ac, j) => (
                      ...
                      <div>
                        ...
                        <div
                          ...
                          <div style={{ fontSize: 13, fontWeight: 500 }}>Загрузить скилл</div>
                          <div style={{ fontSize: 11, color: C.tt }}>PDF, MD, TXT, JSON</div>
                        </div>
                        ...
                        {sks.map(s => (
                          <div key={s.id} style={{
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              <div style={{ fontSize: 11, color: C.tt }}>{s.sz} · {s.rl} правил</div>
                            </div>
                  ...
                  {subTab === "versions" && (
                    <div>
                      {vers.length === 0 ? (
                        <div style={{ textAlign: "center", padding: 24, color: C.tt, fontSize: 13 }}>История версий появится после первого обновления скиллов</div>
                      ) : (
                            ...
                            <div key={j} style={{ position: "relative", marginBottom: 20, paddingLeft: 20 }}>
                              ...
                              <div>
                                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                  <span style={{ fontSize: 14, fontWeight: 500 }}>v{v.v}</span>
                                  {v.s === "current" && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 10, background: a.co + "15", color: a.co }}>Текущая</span>}
                                  {v.s === "bad" && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 10, background: C.dl, color: G.r }}>Ухудшение</span>}
                                </div>
                                <div style={{ fontSize: 12, color: C.tt, marginTop: 2 }}>{v.dt} · {v.ch}</div>
                  ...
                  {subTab === "impact" && (
                    <div>
                      {imp ? (
                          ...
                          <div style={{ background: C.bg, borderRadius: 12, padding: 16 }}>
                            <ResponsiveContainer width="100%" height={140}>
                              <LineChart data={imp}>
                                ...
                                <XAxis dataKey="w" tick={{ fill: C.tt, fontSize: 11 }} axisLine={false} tickLine={false} />
                                <YAxis tick={{ fill: C.tt, fontSize: 11 }} axisLine={false} tickLine={false} />
                                <Tooltip contentStyle={{ background: "#FFF", border: "1px solid " + C.bd, borderRadius: 8, fontSize: 12 }} />
                        ...
                        <div style={{ textAlign: "center", padding: 24, color: C.tt, fontSize: 13 }}>
